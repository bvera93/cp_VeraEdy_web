...
class RattingController extends Controller
{
	    ...
	    ->first();
  		try {
		    ...
		    if($data=="") {
		    	...
		    	return response()->json(['status'=>1,'message'=>'Su Calificación ha sido enviada'],200);
	        } else {
	            return response()->json(['status'=>0,'message'=>'Ya se ha enviado.'],400);
	        }
    ...
    public function rattinglist(Request $request)
    {
        ...
        if(!empty($data))
        {
        	return response()->json(['status'=>1,'message'=>'Lista de Calificaciones','data'=>$data],200);
        }
        else
        {
